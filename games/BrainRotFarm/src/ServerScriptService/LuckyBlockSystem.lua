...
--
-- Flux :
--   1. Achat -> modele "Lucky Block" du tier dans le carry (Main : grantLuckyBlock)
...
--
-- NOTE : module porte depuis LavaTower. Le shared-lib (DropSystem / CarrySystem /
-- BrainrotPositioner / Logger) etant commun aux deux jeux, le pipeline est identique.
-- Seule la config (Config.Shop.LuckyBlocks) et le require de GameConfig different.
...
local function jouerAnimation(player, touchPart, sourcesTier, sourceResultat, lbYaw)
    ...
    -- Phase 3 : donner le Brainrot resultat au joueur comme un vrai pickup
