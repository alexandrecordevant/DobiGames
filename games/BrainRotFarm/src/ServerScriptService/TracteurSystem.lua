...
-- Activé uniquement si upgrade Tracteur acheté (ou TracteurDevActif = true)
...
local MARGE_MUR        = Config.TracteurMargeMur          or 9   -- studs de marge avant les murs
local ROTATION_OFFSET  = CFrame.Angles(0, math.rad(Config.TracteurRotationOffsetDeg or -90), 0)
...
local function calculerLignes(spawnZone)
    ...
    -- Bornes du champ (avec marge pour éviter les murs)
    local zMin = math.min(wallTop.Position.Z,    wallBottom.Position.Z) + MARGE_MUR
    local zMax = math.max(wallTop.Position.Z,    wallBottom.Position.Z) - MARGE_MUR
    local xMin = math.min(wallLeft.Position.X,   wallRight.Position.X) + MARGE_MUR
    local xMax = math.max(wallLeft.Position.X,   wallRight.Position.X) - MARGE_MUR
    local y    = math.max(
...
-- ============================================================
-- Collecte automatique des BRs proches du tracteur
-- ============================================================
