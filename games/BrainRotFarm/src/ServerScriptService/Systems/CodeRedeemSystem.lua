...
CodeRedeemSystem.SeedInventory = nil  -- module SeedInventory
...
-- ============================================================
-- Distribution des graines (même logique que ClaimDailySeed dans Main)
-- ============================================================
...
local function donnerGraines(player, data, seeds)
    ...
    local SeedInventory = CodeRedeemSystem.SeedInventory
    local CarrySystem   = CodeRedeemSystem.CarrySystem
    if not SeedInventory or not CarrySystem then return end

    -- Dossier CarriedSeeds sur le joueur (source de vérité pour le PlantSeed)
    ...
    for _, entree in ipairs(seeds) do
        local rarete   = entree.Rarity
        local quantite = entree.Quantity or 1

        SeedInventory.Add(data, rarete, quantite)

        for _ = 1, quantite do
            -- StringValue dans CarriedSeeds (lu par le prompt "Plant" du pot)
            local sv   = Instance.new("StringValue")
            sv.Value   = rarete
            sv.Parent  = carriedSeeds

            -- Tool visuel dans le Backpack du joueur
            if backpack then
                ...
                handle.Parent       = tool
                local lumiere       = Instance.new("PointLight", handle)
...
local function appliquerRecompenses(player, data, rewards)
    ...
    if rewards.Seeds and #rewards.Seeds > 0 then
        donnerGraines(player, data, rewards.Seeds)
    end
...
local function construireMessage(rewards)
    ...
    if rewards.Seeds then
        for _, s in ipairs(rewards.Seeds) do
            table.insert(parties, "+" .. (s.Quantity or 1) .. " " .. s.Rarity .. " Seed")
        end
    end
