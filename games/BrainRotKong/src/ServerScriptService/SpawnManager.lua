...
-- BrainRotKong — Spawn champs individuels (COMMON → BRAINROT_GOD)
...
-- (Roll bonus Tracteur retiré — Kong)
	...
	task.spawn(function()
		...
		if clone and clone.Parent then
			...
			if SpawnManager.OnBRSpawned then
				...
				if ordreRarete >= 4 then  -- EPIC+ → leaderboard
...
-- (SetSpawnRateMultiplier / Arroseur retiré — Kong)
...
-- Supprime un BR actif par id + baseIndex
