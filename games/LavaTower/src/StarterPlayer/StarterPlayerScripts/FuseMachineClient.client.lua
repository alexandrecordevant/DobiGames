-- StarterPlayerScripts/FuseMachineClient.client.lua
-- UI Fuse Machine LavaTower -- rework visuel
-- Logique inchangee : slots, selection carry, resultat, timer
...
local barreResultatGauche  -- bordure gauche de la carte resultat
...
local function creerUI()
    ...
    -- Cadre principal 480x560
    cadre = Instance.new("Frame")
    cadre.Name                   = "Cadre"
    cadre.Size                   = UDim2.new(0, 480, 0, 560)
    ...
    -- ─── Header ──────────────────────────────────────────────────────────────
    -- Bande d'accent orange sur le bord gauche du header
    ...
    -- ─── FRAME SELECTION ─────────────────────────────────────────────────────
    -- Padding lateral 12px de chaque cote
    ...
    creerSectionLabel(frameSelection, "INGREDIENTS  --  4 Brainrots requis", 0)
    ...
    for i = 1, 4 do
        ...
        -- Icone / lettre rarete (zone centrale)
        ...
        -- Bandeau bas avec numero de slot
    ...
    sep2.Position         = UDim2.new(0, 0, 0, 128)
    sep2.BackgroundColor3 = C_BORDURE
    sep2.BorderSizePixel  = 0
    sep2.Parent           = frameSelection

    -- Section RESULTAT (y=133)
    creerSectionLabel(frameSelection, "RESULTAT POSSIBLE", 133)

    -- Carte resultat avec bordure gauche coloree (y=155, h=56)
    local resultFrame = Instance.new("Frame")
    resultFrame.Name                   = "ResultFrame"
    resultFrame.Size                   = UDim2.new(1, 0, 0, 56)
    resultFrame.Position               = UDim2.new(0, 0, 0, 155)
    ...
    labelResultat.Text                   = "<font color='#828282'>Selectionnez 4 Brainrots...</font>"
    ...
    -- Cout + duree dans un fond separe (y=215, h=26)
    local coutFrame = Instance.new("Frame")
    coutFrame.Size                   = UDim2.new(1, 0, 0, 28)
    coutFrame.Position               = UDim2.new(0, 0, 0, 215)
    ...
    -- Section VOS BRAINROTS (y=247)
    creerSectionLabel(frameSelection, "VOS BRAINROTS  --  Cliquez pour ajouter", 247)

    -- ScrollingFrame carry (y=269, h=110)
    carryFrame = Instance.new("ScrollingFrame")
    carryFrame.Name                   = "CarryFrame"
    carryFrame.Size                   = UDim2.new(1, 0, 0, 112)
    carryFrame.Position               = UDim2.new(0, 0, 0, 269)
    ...
    -- Bandeau de statut orange fonce
    ...
    -- Conteneur barre de progression (y=92, h=28)
    ...
    lRappel.Text                   = "Revenez avec le ProximityPrompt quand c'est pret."
...
rafraichirSlot = function(i)
    ...
    if tool and tool.Parent then
        ...
        if numStrip then
            numStrip.BackgroundColor3 = Color3.new(coul.R * 0.3, coul.G * 0.3, coul.B * 0.3)
        end
    else
...
mettreAJourRecette = function()
    ...
    if not recetteTrouvee then
        if labelResultat then
            labelResultat.Text = "<font color='#e07070'>Combinaison invalide</font>"
        end
        if barreResultatGauche then barreResultatGauche.BackgroundColor3 = C_INVALIDE end
        if labelCout then labelCout.Text = "" end
        if btnLancer then
            btnLancer.BackgroundColor3 = C_BTN_OFF
            btnLancer.Text             = "Combinaison invalide"
            btnLancer.TextColor3       = C_TEXTE2
            btnLancer.Font             = Enum.Font.Gotham
            btnLancer.TextSize         = 14
        end
    ...
    -- Sorties possibles avec couleurs raretes
    local parties = {}
    ...
    if labelCout then
        labelCout.Text = "<font color='#ffd050'>Cout : " .. recetteTrouvee.cout
            .. " coins</font>   <font color='#828282'>|   Duree : 1h 30m</font>"
    end
    ...
    for _, tool in ipairs(backpack:GetChildren()) do
        if tool:IsA("Tool") then
            ...
            if rarete then
                ...
                -- Barre de couleur rarete en haut
...
local function demarrerTimerClient(debutFusion, dureeFusion)
    ...
    timerConn = RunService.Heartbeat:Connect(function()
        ...
        if restant <= 0 then
            if labelTimer then
                labelTimer.Text       = "PRET A COLLECTER !"
...
local function nettoyerDoublons()
    for _, child in ipairs(playerGui:GetChildren()) do
        if child.Name == "FuseMachineUI" and child ~= screenGui then
            child:Destroy()
        end
    end
end
nettoyerDoublons()
task.defer(nettoyerDoublons)
